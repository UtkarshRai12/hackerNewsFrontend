...
import { StoryCard } from "../storyComponents/storyCard";
const STORIES = gql(`
...
export const NewStoriesContainer:React.FC=()=>{
    const {count}= useInfiniteScroll();
    const { loading, error, data } = useQuery(STORIES, {
        variables: { type: "new" }, // Here, 'type' is the variable name and 'top' is the value
      });
    if (loading) return(
    ...
    return(
        ...
        <Badge className="flex justify-between items-center text-white-700" variant="secondary">
            <p className="text-2xl text-center py-10">New Stories</p>
        </Badge>
          {data.getStories.slice(0, count).map((story:Story) => (
          <StoryCard storyContent={story} />
        ))}
        </div>
